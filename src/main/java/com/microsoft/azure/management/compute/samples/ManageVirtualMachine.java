...
import com.microsoft.azure.management.samples.JavaAuthenticator;
import com.microsoft.azure.management.samples.KeyCert;
...
public final class ManageVirtualMachine {
    ...
    public static void main(String[] args) {
        try {
            ...
            final String key = System.getenv("AZURE_CERT_SECRET");
            final String subscriptionId = System.getenv("AZURE_SUBSCRIPTION_ID");
            final String certPath = System.getenv("AZURE_CERT_PATH");
            ...
            // Retrieve certificate info for authentication
            final KeyCert certificateKey = JavaAuthenticator.readPfx(certPath, key);
            final byte[] certValue = certificateKey.getCertValue();
            final PrivateKey privateKey = certificateKey.getKey();
            
            // Authenticate to Azure Stack using Service Principal credentials
            AzureTokenCredentials credentials = new ApplicationTokenCredentials(client, tenant, certValue, key, AZURE_STACK)
